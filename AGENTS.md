...
- Prospects: `src/lib/prospects/explorium.ts` calls Explorium's API (`EXPLORIUM_API_KEY`, server only). Filter values in `schema.ts` are Explorium's enums, sent as-is. Results live in the browser (sessionStorage), not the database; "Add to Sales" creates opportunities.
