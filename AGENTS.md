...
- Generated images (Reel cover + scenes, `src/lib/engine/images.ts`, model `OPENAI_IMAGE_MODEL`) are stored in the private Supabase Storage bucket `unison-media` under `<workspace>/<generation>/reel/`, created on first use and served with signed URLs (`src/lib/media/storage.ts`). Deleting a set deletes its images. The brand's visual style (`visual_*` columns, migration 0003) shapes every image; code tolerates the columns being missing.
