...
- Stack: Next.js 16 App Router, Supabase (Postgres, email-link auth, row-level security), OpenAI Responses API with Zod structured outputs, next-intl (EN/ES, cookie-based, no URL prefixes), shadcn/ui (Radix) + Tailwind v4.
- Every Server Action re-checks the caller with `getReadyContext()` and that the brand/workspace is theirs; RLS in `supabase/migrations` is the second line of defense. Child tables carry `(brand_id, workspace_id)` with a composite foreign key so rows can't point at another workspace's brand.
